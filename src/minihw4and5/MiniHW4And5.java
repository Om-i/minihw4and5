...
//        Class.forName("com.mysql.cj.jdbc.Driver").newInstance();
         ...
         *
         */
        Connection conn = DriverManager.getConnection(url + "?allowLoadLocalInfile=true" , userName , passWord); // The url query string referencing the driver setting for local data upload
        Statement stmt = conn.createStatement();
        conn.setAutoCommit(false); // prevents database interaction before all the queries are loaded
        /*
         *
         */
        Scanner sc = new Scanner(new File(sqlPath)).useDelimiter("(?<=;)"); // Reads the file in sections delimited by a semiclumn, the regex prevents the delimiter removal from the string
        while (sc.hasNext()) {
            stmt.addBatch(sc.next()); // Add a single query to the statement batch.
        }
        /*
         *
         */
